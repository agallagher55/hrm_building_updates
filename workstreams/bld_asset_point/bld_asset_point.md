...
- [x] Get confirmation on `FCODE` usage from Digital Services. Everything else in this section, `DM_01.BUILDING.DIM_BUILDING` / `DIM_BUILDING_NEW` usage and whether anything downstream reads it, is folded into the notification email in §4 rather than being separate legwork here. **Confirmed 2026-08-20** (Somya Agarwal, reply to the §4 notice): "FCODE is not utilized in any downstream processes." No further action needed on this question.
...
- **17 fields being deleted:** Confirmed safe. "These fields are not being used in our processes and can be safely removed. We would like to give it a test run in QA once it is updated on your end." Cross-checked against the field list she attached (`BLD_building_assetpoint_Fields_ETL_Utilization.xlsx`, saved to `../../changes/`, also in this repo at `../../changes/BLD_building_assetpoint_Fields_ETL_Utilization.xlsx`) — all 17 delete candidates (`REPLCSTOTL`, `MAT`, `MATCONF`, `LANDID`, `ASSETRAW`, `ASSETDESC`, `CRIT`, `CRITCONF`, `RMLIFECONF`, `INSTCSCONF`, `REPLCSRA`, `REPLRACONF`, `REPLCSCONF`, `TCACAT`, `PERFRMRA`, `PERFRMCONF`, `PROFCNCAT`) are marked blank (not "Yes") in her "Used in ETL" column, confirming her answer independently. This satisfies Pre-Work §1's "Confirm none of the 17 fields being deleted are referenced" check from the ETL side, complementing the schema-side confirmation already done there.
- **`FCODE`:** Confirmed not used downstream ("FCODE is not utilized in any downstream processes"). Also blank in her attached field list. See §3 above, now resolved.

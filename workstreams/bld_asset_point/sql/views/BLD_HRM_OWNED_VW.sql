...
-- It joins BLD_BUILDING_POLYGON to BLD_HRM_OWNED_FINAL on BL_ID.
-- Flagged as impacted by TASK0326632 in the RFC's IMPACTS tab, so worth
-- confirming whether BLD_HRM_OWNED_FINAL is itself derived from
-- BLD_building_assetpoint before assuming this view needs changes.

CREATE VIEW sdeadm.BLD_HRM_OWNED_VW
AS
SELECT   P.OBJECTID, P.SHAPE, P.BL_ID, A.BL_NAME, A.OWNER,
         ...
         A.STR_TYPE, A.GSA_NAME, A.USE_ID
FROM     sdeadm.BLD_BUILDING_POLYGON AS P INNER JOIN
         sdeadm.BLD_HRM_OWNED_FINAL AS A ON P.BL_ID = A.BL_ID
